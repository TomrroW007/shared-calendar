...
export function parseQuickAddCommand(text) {
    ...
    // V3.0 Intent Mapping
    if (cleanText.includes('聚餐') || cleanText.includes('饭') || cleanText.includes('火锅')) {
        result.vibe = 'party';
        result.emoji = '🍕';
    } else if (cleanText.includes('咖啡') || cleanText.includes('聊')) {
        result.vibe = 'chill';
        result.emoji = '☕';
    } else if (cleanText.includes('加班') || cleanText.includes('会')) {
        result.vibe = 'focus';
        result.emoji = '💼';
    }
